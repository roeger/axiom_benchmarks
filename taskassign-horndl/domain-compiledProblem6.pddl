...
(:action hireElectronicEng
  ...
  :effect (when (forall (?x1 ?x2 ?x3) (AUX9 ?x1 ?x2 ?x3)) (electronicengineer ?n)))
(:action hireInformaticEng
  :parameters (?n)
  :precondition (not (DATALOG_INCONSISTENT))
  :effect (when (forall (?x1 ?x2 ?x3) (AUX10 ?x1 ?x2 ?x3)) (informaticengineer ?n)))
