...
(:action hireElectronicEng
  ...
  :effect (when (forall (?x1 ?x2 ?x3) (AUX9 ?x2 ?x3 ?x1)) (electronicengineer ?n)))
(:action hireInformaticEng
  :parameters (?n)
  :precondition (not (DATALOG_INCONSISTENT))
  :effect (when (forall (?x1 ?x2 ?x3) (AUX10 ?x2 ?x3 ?x1)) (informaticengineer ?n)))
