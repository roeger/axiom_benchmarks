...
(define (domain sokoban-sequential)
  (:requirements :typing :derived-predicates :adl :action-costs)
  (:types thing location direction - object
          player stone - thing)

  (:predicates
   (clear ?l - location)
   (notat ?s - stone ?l - location)
  ...
  (:derived (notat ?s - stone ?l - location)
	    (not (at ?s ?l)))
  ...
  (:derived (clear ?l - location) (forall (?s - stone) (notat ?s ?l)))
